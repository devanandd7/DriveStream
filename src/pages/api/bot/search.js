...
export default async function handler(req, res) {
  try {
    const { tg_id, q, limit, pageToken } = req.query || {};
    ...
    const where = `trashed=false and (name contains '${safeQ}' or fullText contains '${safeQ}')`;
    const qParam = encodeURIComponent(where);
    const fields = encodeURIComponent(
      "nextPageToken, files(id,name,mimeType,webViewLink,webContentLink,thumbnailLink,iconLink,hasThumbnail,modifiedTime,size)"
    );
    const pageSize = Math.max(1, Math.min(parseInt(limit || "10", 10), 100));
    const base = `https://www.googleapis.com/drive/v3/files?q=${qParam}&fields=${fields}&pageSize=${pageSize}&orderBy=modifiedTime desc&includeItemsFromAllDrives=true&supportsAllDrives=true`;
