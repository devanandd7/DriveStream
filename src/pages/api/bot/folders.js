...
function alphaRange(where, range) {
  // range: 'A-F', 'G-L', 'M-R', 'S-Z'
  const map = {
    "A-F": ["A", "G"],
    "G-L": ["G", "M"],
    "M-R": ["M", "S"],
  ...
  const pair = map[range];
  if (!pair) return where;
  const [start, end] = pair;
  // Drive supports inequalities on name
  return `${where} and name >= '${start}' and name < '${end}'`;
}

export default async function handler(req, res) {
  try {
    const { tg_id, limit, pageToken, range } = req.query || {};
    ...
    let where = "mimeType = 'application/vnd.google-apps.folder' and trashed=false";
    if (range) where = alphaRange(where, String(range));

    const fields = encodeURIComponent(
      "nextPageToken, files(id,name,mimeType,webViewLink,iconLink,thumbnailLink,hasThumbnail,modifiedTime,size)"
    );
    const pageSize = Math.max(1, Math.min(parseInt(limit || "8", 10), 100));
    const base = `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(where)}&fields=${fields}&pageSize=${pageSize}&orderBy=name asc&includeItemsFromAllDrives=true&supportsAllDrives=true`;
